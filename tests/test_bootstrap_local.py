...
def _write_models_and_dataset(tmp_path: Path, monkeypatch, model_name: str) -> None:
    ...
    (models_dir / model_name).write_text("gguf", encoding="utf-8")
...
def test_ollama_validation_skips_local_model_check(tmp_path, monkeypatch) -> None:
    # No GGUF written; ollama pulls from HF, so model files are not required.
    deployment_dir = tmp_path / "deployment"
    ...
    env = {"CHROMA_AUTH_TOKEN": "a-real-token"}
